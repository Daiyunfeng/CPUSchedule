...
/**
 * 扩展只需要实现schedule operatingSequences中只需要保存进程信息 中间调度空闲时间不需要
...
public abstract class AbstractSchedule
{
	...
	protected List<OperatingSequence> operatingSequences; // 只需要记录每个进程在什么时候运行什么时候结束就好
															// 不需要考虑空余
	...
	public void paintResult()
	{
		...
		{
			System.out.println(progresses.get(i).getName() + " 到达时间:" + progresses.get(i).getArrivedTime() + " 优先级"
					+ progresses.get(i).getPriority() + " 运行时间" + progresses.get(i).getRunTime());
